// nvcc cuBLAS.cu -o a1 -L /usr/local/cuda/lib64 -lcudart -lcuda -lcublas
#include <sys/time.h>
                   ...
                   S *alpha, S *beta, int algo)
{
    ...
    status = cublasGemmEx(handle, transA, transB,
                          m, n, k,              
                          alpha, A, AType, lda,
                          B, BType, ldb, beta,
                          C, CType, ldc,
               ...
               S *alpha, S *beta, int algo, int iteration)
{
    ...
    for (int i = 0; i < iteration; ++i)
    {
        ...
        // AB = (BTAT)T
        int success = cublas_gemm_ex(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                                     n, m, k,
                                     B, A, C,
                                     n, k, n,
                                     alpha, beta,
...
int main()
{
    ...
    cublasCreate(&handle);
    // algo -1到23表示不使用tensor core算法的结果，algo 99到115表示使用tensor core算法的结果
    printf(">>>>>>>>>>>>>>>>> test fp32 >>>>>>>>>>>>>>>>>\n");
