...
int main(int argc, char **argv)
{
    ...
    float *h_C_my = (float *)malloc(bytes_C);
    ...
    for (int run = 0; run < nIter; run++)
    {
        ...
        Sgemm_my<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y, THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER>
    ...
    CHECK_CUDA(cudaMemcpy(h_C_my, d_C, bytes_C, cudaMemcpyDeviceToHost));
    ...
    for (int i = 0; i < M * N; i++)
    {
        int row = i / N;
        int col = i % N;
        double abs_err = fabs(h_C_CUBLAS[i] - h_C_my[col * M + row]);
        double dot_length = M;
        double abs_val = fabs(h_C_my[i]);
        double rel_err = abs_err / abs_val / dot_length;
        if (rel_err > eps)
        {
            printf("Error! Matrix[%05d]=%.8f, ref=%.8f error term is > %E\n",
                   i, h_C_my[i], h_C_CUBLAS[col * M + row], eps);
            correct = false;
            break;
        }
    }
